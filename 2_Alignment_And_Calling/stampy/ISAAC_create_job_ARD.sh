...
echo '#!/bin/sh' > $name.sh 
echo '#' >> $name.sh
echo '#$ -N '$name'_ISAAC_aligner' >> $name.sh
echo '#$ -S /bin/bash' >> $name.sh 
echo '#$ -cwd' >> $name.sh 
echo '#$ -j y' >> $name.sh 
echo '#$ -pe multicore 8' >> $name.sh 
echo '#$ -R y' >> $name.sh
echo '#$ -o /dev/ngs004/hhong/CQ/wf/ISAAC/'$name'.log' >> $name.sh
echo '' >> $name.sh
echo '# datasets' >> $name.sh
echo 'sample='$name'' >> $name.sh
echo 'F=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170403_DNA_ILM_ARD/'$name'' >> $name.sh  # please check data diectory
...
echo 'export isaac=$ztools/isaac_aligner/bin' >> $name.sh
...
echo 'export mybin=$JAVA_HOME/bin:$ztools/isaac_aligner/bin:$ztools/GATK3.7:$ztools/bcftools-1.3.1:$ztools/samtools-1.3.1:$ztools/SNVer-0.5.3:$ztools/isaac_variant_caller-1.0.7/etc:$ztools/isaac_variant_caller-1.0.7/bin:$ztools/gvcftools-0.14/bin' >> $name.sh
...
echo 'isaacindex=/dev/ngs004/hhong/CQ/hg38/ISAAC' >> $name.sh
echo '' >> $name.sh
echo '# create tempdir for middle results' >> $name.sh
echo 'mkdir /dev/ngs004/hhong/CQ/wf/ISAAC/'$name'' >> $name.sh
echo 'outdir=/dev/ngs004/hhong/CQ/wf/ISAAC/'$name'' >> $name.sh
echo 'mkdir /dev/ngs004/hhong/CQ/wf/ISAAC/'$name'/tempdir' >> $name.sh
echo 'tempdir=/dev/ngs004/hhong/CQ/wf/ISAAC/'$name'/tempdir' >> $name.sh
echo '# ISAAC align' >> $name.sh
echo 'echo ''ISAAC alignment start at time'' && echo "`date`"' >> $name.sh	
echo 'isaac-align -r $isaacindex/sorted-reference.xml -b $F --base-calls-format fastq-gz -j 8 --verbosity 3  --stop-at Bam --keep-unaligned back --realign-gaps yes -o $outdir -m 64 -t $tempdir/Temp  --stats-image-format none --variable-read-length yes;' >> $name.sh
echo 'echo ''ISAAC alignment finish at time'' && echo "`date`"' >> $name.sh	
echo '' >> $name.sh
...
echo 'I=$outdir/Projects/default/default/sorted.bam \' >> $name.sh
