...
#$ -pe multicore 8
#$ -R y
#$ -o /dev/ngs004/hhong/CQ/wf/BWA/WUX_LCL6_1.log

# datasets
sample=WUX_LCL6_1
F01=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170216_DNA_ILM_WUX/Quartet_DNA_ILM_WUX_LCL6_1_20170216_R1.fastq.gz
F02=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170216_DNA_ILM_WUX/Quartet_DNA_ILM_WUX_LCL6_1_20170216_R2.fastq.gz
## set path
...
mkdir /dev/ngs004/hhong/CQ/wf/BWA/WUX_LCL6_1/tempdir
...
--dbsnp $dbsnp \
-stand_call_conf 30 \
-I $outdir/WUX_LCL6_1.aln.sorted.dup.RG.bam \
-o $outdir/WUX_LCL6_1.HC.raw.vcf
...
--dbsnp $dbsnp \
-stand_call_conf 30 \
-I $outdir/WUX_LCL6_1.aln.sorted.dedup.realigned.recal.bam \
-o $outdir/WUX_LCL6_1.HC.GATK.raw.vcf
echo GATK HaplotypeCaller finish at time && echo "`date`"
# Samtools calling
echo samtools mpileup start at time && echo "`date`"
samtools mpileup -ugf $ref $outdir/WUX_LCL6_1.aln.sorted.dup.RG.bam | bcftools call -vmO v -o $outdir/$sample.Samtools.raw.vcf && echo This-Work-is-Done!
echo samtools mpileup finish at time && echo "`date`"
#########################
echo samtools mpileup start at time && echo "`date`"
samtools mpileup -ugf $ref $outdir/WUX_LCL6_1.aln.sorted.dedup.realigned.recal.bam | bcftools call -vmO v -o $outdir/$sample.Samtools.GATK.raw.vcf && echo This-Work-is-Done!
echo samtools mpileup finish at time && echo "`date`"
# Varscan calling
echo Varscan calling start at time && echo "`date`"
samtools mpileup -B -f $ref $outdir/WUX_LCL6_1.aln.sorted.dup.RG.bam | java -Djava.io.tmpdir=$tempdir -Xmx40g -jar $VarScan mpileup2cns --p-value 0.05 --variants 1 --output-vcf 1 > $outdir/WUX_LCL6_1.Varscan.raw.vcf
echo Varscan calling finish at time && echo "`date`"
#########################
echo Varscan calling start at time && echo "`date`"
samtools mpileup -B -f $ref $outdir/WUX_LCL6_1.aln.sorted.dedup.realigned.recal.bam | java -Djava.io.tmpdir=$tempdir -Xmx40g -jar $VarScan mpileup2cns --p-value 0.05 --variants 1 --output-vcf 1 > $outdir/WUX_LCL6_1.Varscan.GATK.raw.vcf
echo Varscan calling finish at time && echo "`date`"
# SNVer calling
...
-i $outdir/WUX_LCL6_1.aln.sorted.dedup.realigned.recal.bam \
-r $ref \
-o $outdir/WUX_LCL6_1_G \
-p 0.05
echo SNVer finish at time && echo "`date`"
# FreeBayes
echo FreeBayes caller start at time && echo "`date`"
$FreeBayes -f $ref -X -u -v -m 30 -q 5 -= $outdir/WUX_LCL6_1.FreeBayes.raw.vcf $outdir/WUX_LCL6_1.aln.sorted.dup.RG.bam
echo FreeBayes caller finish at time && echo "`date`"
#########################
echo FreeBayes caller start at time && echo "`date`"
$FreeBayes -f $ref -X -u -v -m 30 -q 5 -= $outdir/WUX_LCL6_1.FreeBayes.GATK.raw.vcf $outdir/WUX_LCL6_1.aln.sorted.dedup.realigned.recal.bam
echo FreeBayes caller finish at time && echo "`date`"
# ISAAC caller
echo ISAAC calling start at time && echo "`date`"
$issac_caller --BAM=$outdir/WUX_LCL6_1.aln.sorted.dup.RG.bam --ref=$ref --config=$config_file --output-dir=$outdir/myAnalysis
cd $outdir/myAnalysis
make -j 8
echo ISAAC calling finish at time && echo "`date`"
gzip -dc $outdir/myAnalysis/results/WUX_LCL6_1.aln.sorted.dup.RG.genome.vcf.gz | extract_variants > $outdir/WUX_LCL6_1.ISAAC.raw.vcf
#########################
echo ISAAC calling start at time && echo "`date`"
$issac_caller --BAM=$outdir/WUX_LCL6_1.aln.sorted.dedup.realigned.recal.bam --ref=$ref --config=$config_file --output-dir=$outdir/myAnalysis
cd $outdir/myAnalysis
make -j 8
echo ISAAC calling finish at time && echo "`date`"
gzip -dc $outdir/myAnalysis/results/WUX_LCL6_1.aln.sorted.dedup.realigned.recal.genome.vcf.gz | extract_variants > $outdir/WUX_LCL6_1.ISAAC.GATK.raw.vcf
