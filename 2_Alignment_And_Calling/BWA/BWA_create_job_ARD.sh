...
echo '#!/bin/sh' > $name.sh 
echo '#' >> $name.sh
echo '#$ -N '$name'_BWA_aligner' >> $name.sh
echo '#$ -S /bin/bash' >> $name.sh 
echo '#$ -cwd' >> $name.sh 
echo '#$ -j y' >> $name.sh 
echo '#$ -pe multicore 8' >> $name.sh 
echo '#$ -R y' >> $name.sh
echo '#$ -o /dev/ngs004/hhong/CQ/wf/BWA/'$name'.log' >> $name.sh
echo '' >> $name.sh
echo '# datasets' >> $name.sh
echo 'sample='$name'' >> $name.sh
echo 'F01=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170403_DNA_ILM_ARD/Quartet_DNA_ILM_'$name'_20170403_R1.fastq.gz' >> $name.sh  # please check data diectory
echo 'F02=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170403_DNA_ILM_ARD/Quartet_DNA_ILM_'$name'_20170403_R2.fastq.gz' >> $name.sh # please check data diectory
...
echo 'export bwa=$ztools/bwa-0.7.15' >> $name.sh
...
echo 'export mybin=$JAVA_HOME/bin:$ztools/bwa-0.7.15:$ztools/GATK3.7:$ztools/bcftools-1.3.1:$ztools/samtools-1.3.1:$ztools/SNVer-0.5.3:$ztools/isaac_variant_caller-1.0.7/etc:$ztools/isaac_variant_caller-1.0.7/bin:$ztools/gvcftools-0.14/bin' >> $name.sh
...
echo 'bwaindex=/dev/ngs004/hhong/CQ/hg38/BWA/hg38.fasta' >> $name.sh
echo '' >> $name.sh
echo '# create tempdir for middle results' >> $name.sh
# echo 'mkdir /dev/ngs004/hhong/CQ/wf/BWA/'$name'' >> $name.sh
echo 'outdir=/dev/ngs004/hhong/CQ/wf/BWA/'$name'' >> $name.sh
echo 'mkdir /dev/ngs004/hhong/CQ/wf/BWA/'$name'/tempdir' >> $name.sh
echo 'tempdir=/dev/ngs004/hhong/CQ/wf/BWA/'$name'/tempdir' >> $name.sh
# echo '# BWA align' >> $name.sh
# echo 'echo ''BWA alignment start at time'' && echo "`date`"' >> $name.sh	
# echo 'bwa mem -t 8 -M $bwaindex $F01 $F02 > $outdir/$sample.aln.sam' >> $name.sh
# echo 'echo ''BWA alignment finish at time'' && echo "`date`"' >> $name.sh	
echo '' >> $name.sh
