#!/bin/sh
#
#$ -N ARD_LCL6_3_bowtie2_aligner
#$ -S /bin/bash
#$ -cwd
#$ -j y
#$ -pe multicore 8
#$ -R y
#$ -o /dev/ngs004/hhong/CQ/wf/Bowtie2/ARD_LCL6_3.log

# datasets
sample=ARD_LCL6_3
F01=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170403_DNA_ILM_ARD/Quartet_DNA_ILM_ARD_LCL6_3_20170403_R1.fastq.gz
F02=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170403_DNA_ILM_ARD/Quartet_DNA_ILM_ARD_LCL6_3_20170403_R2.fastq.gz
## set path
...
mkdir /dev/ngs004/hhong/CQ/wf/Bowtie2/ARD_LCL6_3
outdir=/dev/ngs004/hhong/CQ/wf/Bowtie2/ARD_LCL6_3
mkdir /dev/ngs004/hhong/CQ/wf/Bowtie2/ARD_LCL6_3/tempdir
tempdir=/dev/ngs004/hhong/CQ/wf/Bowtie2/ARD_LCL6_3/tempdir
...
cp $outdir/ARD_LCL6_3.aln.sorted.dup.RG.bai $outdir/ARD_LCL6_3.aln.sorted.dup.RG.bam.bai
cp $outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bai $outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bam.bai
## SNV calling
mkdir $outdir/ARD_LCL6_3_G
...
--dbsnp $dbsnp \
-stand_call_conf 30 \
-I $outdir/ARD_LCL6_3.aln.sorted.dup.RG.bam \
-o $outdir/ARD_LCL6_3.HC.raw.vcf
...
--dbsnp $dbsnp \
-stand_call_conf 30 \
-I $outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bam \
-o $outdir/ARD_LCL6_3.HC.GATK.raw.vcf
echo GATK HaplotypeCaller finish at time && echo "`date`"
# Samtools calling
echo samtools mpileup start at time && echo "`date`"
samtools mpileup -ugf $ref $outdir/ARD_LCL6_3.aln.sorted.dup.RG.bam | bcftools call -vmO v -o $outdir/$sample.Samtools.raw.vcf && echo This-Work-is-Done!
echo samtools mpileup finish at time && echo "`date`"
#########################
echo samtools mpileup start at time && echo "`date`"
samtools mpileup -ugf $ref $outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bam | bcftools call -vmO v -o $outdir/$sample.Samtools.GATK.raw.vcf && echo This-Work-is-Done!
echo samtools mpileup finish at time && echo "`date`"
# Varscan calling
echo Varscan calling start at time && echo "`date`"
samtools mpileup -B -f $ref $outdir/ARD_LCL6_3.aln.sorted.dup.RG.bam | java -Djava.io.tmpdir=$tempdir -Xmx40g -jar $VarScan mpileup2cns --p-value 0.05 --variants 1 --output-vcf 1 > $outdir/ARD_LCL6_3.Varscan.raw.vcf
echo Varscan calling finish at time && echo "`date`"
#########################
echo Varscan calling start at time && echo "`date`"
samtools mpileup -B -f $ref $outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bam | java -Djava.io.tmpdir=$tempdir -Xmx40g -jar $VarScan mpileup2cns --p-value 0.05 --variants 1 --output-vcf 1 > $outdir/ARD_LCL6_3.Varscan.GATK.raw.vcf
echo Varscan calling finish at time && echo "`date`"
# SNVer calling
...
-i $outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bam \
-r $ref \
-o $outdir/ARD_LCL6_3_G \
-p 0.05
echo SNVer finish at time && echo "`date`"
# FreeBayes
echo FreeBayes caller start at time && echo "`date`"
$FreeBayes -f $ref -X -u -v $outdir/ARD_LCL6_3.FreeBayes.raw.vcf $outdir/ARD_LCL6_3.aln.sorted.dup.RG.bam
echo FreeBayes caller finish at time && echo "`date`"
#########################
echo FreeBayes caller start at time && echo "`date`"
$FreeBayes -f $ref -X -u -v $outdir/ARD_LCL6_3.FreeBayes.GATK.raw.vcf $outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bam
echo FreeBayes caller finish at time && echo "`date`"
# ISAAC caller
echo ISAAC calling start at time && echo "`date`"
$issac_caller --BAM=$outdir/ARD_LCL6_3.aln.sorted.dup.RG.bam --ref=$ref --config=$config_file --output-dir=$outdir/myAnalysis
cd $outdir/myAnalysis
make -j 8
echo ISAAC calling finish at time && echo "`date`"
gzip -dc $outdir/myAnalysis/results/ARD_LCL6_3.aln.sorted.dup.RG.genome.vcf.gz | extract_variants > $outdir/ARD_LCL6_3.ISAAC.raw.vcf
#########################
echo ISAAC calling start at time && echo "`date`"
$issac_caller --BAM=$outdir/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.bam --ref=$ref --config=$config_file --output-dir=$outdir/myAnalysis
cd $outdir/myAnalysis
make -j 8
echo ISAAC calling finish at time && echo "`date`"
gzip -dc $outdir/myAnalysis/results/ARD_LCL6_3.aln.sorted.dedup.realigned.recal.genome.vcf.gz | extract_variants > $outdir/ARD_LCL6_3.ISAAC.GATK.raw.vcf
