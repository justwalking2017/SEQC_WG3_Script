...
echo 'F01=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20160826_DNA_ILM_RPG/Quartet_DNA_ILM_'$name'_1_20160826_R1.fastq.gz' >> $name.sh  # please check data diectory
echo 'F02=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20160826_DNA_ILM_RPG/Quartet_DNA_ILM_'$name'_1_20160826_R2.fastq.gz' >> $name.sh  # please check data diectory
...
echo 'java -Djava.io.tmpdir=$tempdir -Xmx50g -jar $PICARD SortSam \' >> $name.sh
...
echo 'java -Djava.io.tmpdir=$tempdir -Xmx50g -jar $PICARD MarkDuplicates \' >> $name.sh
...
echo 'java -Djava.io.tmpdir=$tempdir -Xmx50g -jar $PICARD AddOrReplaceReadGroups \' >> $name.sh
...
echo 'java -Djava.io.tmpdir=$tempdir -Xmx50g \' >> $name.sh
...
echo 'java -Djava.io.tmpdir=$outdir/tempdir -Xmx50g \' >> $name.sh
echo '-jar $GATK \' >> $name.sh
...
echo '$FreeBayes -f $ref -X -u -v -m 30 -q 5 -= $outdir/'$name'.FreeBayes.raw.vcf $outdir/'$name'.aln.sorted.dup.RG.bam' >> $name.sh
echo 'echo ''FreeBayes caller finish at time'' && echo "`date`"' >> $name.sh
echo '#########################' >> $name.sh
echo 'echo ''FreeBayes caller start at time'' && echo "`date`"' >> $name.sh
echo '$FreeBayes -f $ref -X -u -v -m 30 -q 5 -= $outdir/'$name'.FreeBayes.GATK.raw.vcf $outdir/'$name'.aln.sorted.dedup.realigned.recal.bam' >> $name.sh
echo 'echo ''FreeBayes caller finish at time'' && echo "`date`"' >> $name.sh
