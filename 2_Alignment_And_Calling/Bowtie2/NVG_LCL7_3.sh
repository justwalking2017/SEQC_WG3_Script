#!/bin/sh
#
#$ -N NVG_LCL7_3_bowtie2_aligner
#$ -S /bin/bash
#$ -cwd
#$ -j y
#$ -pe multicore 8
#$ -R y
#$ -o /dev/ngs004/hhong/CQ/wf/Bowtie2/NVG_LCL7_3.log

# datasets
sample=NVG_LCL7_3
F01=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170329_DNA_ILM_NVG/Quartet_DNA_ILM_NVG_LCL7_3_20170329_R1.fastq.gz
F02=/dev/ngs004/hhong/CQ/data/Quartet/DNA/20170329_DNA_ILM_NVG/Quartet_DNA_ILM_NVG_LCL7_3_20170329_R2.fastq.gz
## set path
...
mkdir /dev/ngs004/hhong/CQ/wf/Bowtie2/NVG_LCL7_3
outdir=/dev/ngs004/hhong/CQ/wf/Bowtie2/NVG_LCL7_3
mkdir /dev/ngs004/hhong/CQ/wf/Bowtie2/NVG_LCL7_3/tempdir
tempdir=/dev/ngs004/hhong/CQ/wf/Bowtie2/NVG_LCL7_3/tempdir
...
cp $outdir/NVG_LCL7_3.aln.sorted.dup.RG.bai $outdir/NVG_LCL7_3.aln.sorted.dup.RG.bam.bai
cp $outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bai $outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bam.bai
## SNV calling
mkdir $outdir/NVG_LCL7_3_G
...
--dbsnp $dbsnp \
-stand_call_conf 30 \
-I $outdir/NVG_LCL7_3.aln.sorted.dup.RG.bam \
-o $outdir/NVG_LCL7_3.HC.raw.vcf
...
--dbsnp $dbsnp \
-stand_call_conf 30 \
-I $outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bam \
-o $outdir/NVG_LCL7_3.HC.GATK.raw.vcf
echo GATK HaplotypeCaller finish at time && echo "`date`"
# Samtools calling
echo samtools mpileup start at time && echo "`date`"
samtools mpileup -ugf $ref $outdir/NVG_LCL7_3.aln.sorted.dup.RG.bam | bcftools call -vmO v -o $outdir/$sample.Samtools.raw.vcf && echo This-Work-is-Done!
echo samtools mpileup finish at time && echo "`date`"
#########################
echo samtools mpileup start at time && echo "`date`"
samtools mpileup -ugf $ref $outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bam | bcftools call -vmO v -o $outdir/$sample.Samtools.GATK.raw.vcf && echo This-Work-is-Done!
echo samtools mpileup finish at time && echo "`date`"
# Varscan calling
echo Varscan calling start at time && echo "`date`"
samtools mpileup -B -f $ref $outdir/NVG_LCL7_3.aln.sorted.dup.RG.bam | java -Djava.io.tmpdir=$tempdir -Xmx40g -jar $VarScan mpileup2cns --p-value 0.05 --variants 1 --output-vcf 1 > $outdir/NVG_LCL7_3.Varscan.raw.vcf
echo Varscan calling finish at time && echo "`date`"
#########################
echo Varscan calling start at time && echo "`date`"
samtools mpileup -B -f $ref $outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bam | java -Djava.io.tmpdir=$tempdir -Xmx40g -jar $VarScan mpileup2cns --p-value 0.05 --variants 1 --output-vcf 1 > $outdir/NVG_LCL7_3.Varscan.GATK.raw.vcf
echo Varscan calling finish at time && echo "`date`"
# SNVer calling
...
-i $outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bam \
-r $ref \
-o $outdir/NVG_LCL7_3_G \
-p 0.05
echo SNVer finish at time && echo "`date`"
# FreeBayes
echo FreeBayes caller start at time && echo "`date`"
$FreeBayes -f $ref -X -u -v $outdir/NVG_LCL7_3.FreeBayes.raw.vcf $outdir/NVG_LCL7_3.aln.sorted.dup.RG.bam
echo FreeBayes caller finish at time && echo "`date`"
#########################
echo FreeBayes caller start at time && echo "`date`"
$FreeBayes -f $ref -X -u -v $outdir/NVG_LCL7_3.FreeBayes.GATK.raw.vcf $outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bam
echo FreeBayes caller finish at time && echo "`date`"
# ISAAC caller
echo ISAAC calling start at time && echo "`date`"
$issac_caller --BAM=$outdir/NVG_LCL7_3.aln.sorted.dup.RG.bam --ref=$ref --config=$config_file --output-dir=$outdir/myAnalysis
cd $outdir/myAnalysis
make -j 8
echo ISAAC calling finish at time && echo "`date`"
gzip -dc $outdir/myAnalysis/results/NVG_LCL7_3.aln.sorted.dup.RG.genome.vcf.gz | extract_variants > $outdir/NVG_LCL7_3.ISAAC.raw.vcf
#########################
echo ISAAC calling start at time && echo "`date`"
$issac_caller --BAM=$outdir/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.bam --ref=$ref --config=$config_file --output-dir=$outdir/myAnalysis
cd $outdir/myAnalysis
make -j 8
echo ISAAC calling finish at time && echo "`date`"
gzip -dc $outdir/myAnalysis/results/NVG_LCL7_3.aln.sorted.dedup.realigned.recal.genome.vcf.gz | extract_variants > $outdir/NVG_LCL7_3.ISAAC.GATK.raw.vcf
