...
die "perl $0 <pos_file> <sample> <lab> <align> <caller> " if($#ARGV!=4);

my($posFile, $sample, $lab, $aligner, $caller)=@ARGV;
...
my $sourdir='/dev/ngs007/hhong/benchmarking/CQ/1_BED_filteredVCF';
my $varFile1=$sourdir.'/'.$aligner.'_'.$caller.'/'.$lab.'_'.$sample.'_1.'.$caller.'.bedFilt.vcf';
my $varFile2=$sourdir.'/'.$aligner.'_'.$caller.'/'.$lab.'_'.$sample.'_2.'.$caller.'.bedFilt.vcf';
my $varFile3=$sourdir.'/'.$aligner.'_'.$caller.'/'.$lab.'_'.$sample.'_3.'.$caller.'.bedFilt.vcf';
#my $outdir='/dev/ngs007/hhong/benchmarking/CQ/3_bedtools_repOut';
my $outdir='/dev/ngs005/scratch/hhong/rep_repro_analysis/CQ/17_snvGT-CS_1119/3_bedtools_repOut';
my $export1=$outdir.'/'.$aligner.'_'.$caller.'/'.$lab.'_'.$sample.'_3repCS.vcf';
my $export2=$outdir.'/'.$aligner.'_'.$caller.'/'.$lab.'_'.$sample.'_3rep.noCS.vcf';
my $export3=$outdir.'/'.$aligner.'_'.$caller.'/'.$lab.'_'.$sample.'_3rep.stat.txt';
...
#		print EXPORN $_."\n";next;
		...
		print EXPORT "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t"."$aligner".'_'."$caller\n";next;
