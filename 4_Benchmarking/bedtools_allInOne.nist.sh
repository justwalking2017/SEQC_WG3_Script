...
#$ -pe hostcore 4
#$ -R y
#$ -o /dev/ngs005/scratch/hhong/rep_repro_analysis/CQ/17_snvGT-CS_1119/8_bedtools_allInOneMrg/bedtools.multiinter.nist.log
...
bedtools multiinter -i $sourcedir/"$1"_HC_8alignCS_3repCS.sort.vcf $sourcedir/"$1"_sentieon_3repCS.sort.vcf $sourcedir/"$1"_FreeBayes_8alignCS_3repCS.sort.vcf $sourcedir/"$1"_Samtools_8alignCS_3repCS.sort.vcf $sourcedir/"$1"_SNVer_8alignCS_3repCS.sort.vcf $sourcedir/"$1"_Varscan_8alignCS_3repCS.sort.vcf  $sourcedir/"$1"_RTG_3repCS.sort.vcf $sourcedir/"$1"_ISAAC_8alignCS_3repCS.sort.vcf   >$outdir/"$1"_callerMrg_alignerMrg_labMrg_repMrg.txt

#perl $outdir/callerVcfFetch_vcfV.update1130.pl $outdir/"$1"_callerMrg_alignerMrg_labMrg_repMrg.txt $1
perl $outdir/callerVcfFetch_vcfV.update1203.nist.v3.pl $outdir/"$1"_callerMrg_alignerMrg_labMrg_repMrg.txt $1
perl /dev/ngs001/bpan/bin/GIAB_bin/vcfsorter.pl /dev/ngs004/hhong/CQ/hg38/hg38.dict $outdir/"$1"_8callerCS_8alignCS_3repCS.vcf >$outdir/"$1"_8callerCS_8alignCS_3repCS.sort.vcf
